...
from chemprop_solvation.data.utils import get_data, get_header
...
def average_duplicates(args):
    ...
    for inchi, targets in zip(data.inchi(), data.targets()):
        smiles_to_targets[inchi].append(targets)
        if len(smiles_to_targets[inchi]) == 1:
            smiles_in_order.append(inchi)

    # Find duplicates
    duplicate_count = 0
    stds = []
    new_data = []
    for inchi in smiles_in_order:
        all_targets = smiles_to_targets[inchi]
    ...
    with open(args.save_path, 'w') as f:
        ...
        for inchi, avg_targets in new_data:
            f.write(inchi + ',' + ','.join(str(value) if value is not None else '' for value in avg_targets) + '\n')
