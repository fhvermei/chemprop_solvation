...
class MPN_solvation(nn.Module):
    ...
    def forward(self,
        ...
        output_solute = self.encoder_solute.forward(batch_solute, features_batch[:, 0] if features_batch else None)
        output_solvent = self.encoder_solvent.forward(batch_solvent, features_batch[:, 1] if features_batch else None)
