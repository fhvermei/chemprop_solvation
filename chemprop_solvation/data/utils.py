...
from chemprop_solvation.features import load_features


def get_task_names(path: str, use_compound_names: bool = False, solvation: bool = False) -> List[str]:
    """
    Gets the task names from a data CSV file.

    :param path: Path to a CSV file.
    :param use_compound_names: Whether file has compound names in addition to smiles strings.
    :param solvation: Whether another smiles is used to represent the solvent
...
def filter_invalid_smiles(data: MoleculeDataset) -> MoleculeDataset:
    ...
    return MoleculeDataset([datapoint for datapoint in data
                            if ((datapoint.smiles != '' and datapoint.mol is not None
                                ...
                                or (datapoint.solvation and datapoint.solvent_smiles != ''
                                and datapoint.solute_smiles != ''
                                and datapoint.solvent_mol is not None and datapoint.solute_mol is not None
                                and datapoint.solvent_mol.GetNumHeavyAtoms() > 0
                                and datapoint.solute_mol.GetNumHeavyAtoms() > 0))])
...
def get_data(path: str,
    ...
    with open(path, encoding='utf-8') as f:
        ...
        for line in reader:
            ...
            if smiles in skip_smiles:
                continue
            if solvation:
                smiles_solvent = line[0]
                smiles_solute = line[1]
                if smiles_solvent in skip_smiles or smiles_solute in skip_smiles:
                    continue
    ...
    if skip_invalid_smiles:
        ...
        if len(data) < original_data_len:
            debug(f'Warning: {original_data_len - len(data)} SMILES or InCHIs are invalid.')
...
def validate_data(data_path: str) -> Set[str]:
    ...
    for smile in tqdm(smiles, total=len(smiles)):
        ...
        if mol is None:
            errors.add('Data includes an invalid SMILES or InChI.')
